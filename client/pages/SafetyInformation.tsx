...
const mockEmergencyContacts: EmergencyContact[] = [
  {
    ...
    phoneNumber: '911',
    description: 'Call for immediate emergency assistance requiring police, fire, or medical response',
    isActive: true,
    priority: 1,
    availableHours: '24/7',
    location: 'Nationwide'
...
const mockSafetyInfo: SafetyInformation[] = [
  {
    ...
    content: 'Protect your home and family with these essential security measures. A secure home starts with good habits and basic security equipment.',
    tips: [
  ...
  },
  {
    ...
    content: 'Stay safe while walking, jogging, or traveling on foot in your community.',
    tips: [
  ...
  },
  {
    ...
    content: 'Stay safe on the roads as a driver, passenger, cyclist, or pedestrian.',
    tips: [
  ...
  },
  {
    ...
    content: 'Be prepared for natural disasters and emergencies with proper planning.',
    tips: [
...
export default function SafetyInformation() {
  const { user } = useAuth();
  const [safetyInfo, setSafetyInfo] = useState<SafetyInformation[]>(mockSafetyInfo);
  const [emergencyContacts, setEmergencyContacts] = useState<EmergencyContact[]>(mockEmergencyContacts);
  const [searchTerm, setSearchTerm] = useState('');
  const [filterCategory, setFilterCategory] = useState<string>('all');
  const [selectedInfo, setSelectedInfo] = useState<SafetyInformation | null>(null);
  ...
  const getCategoryIcon = (category: SafetyCategory) => {
    const icons = {
      ...
      [SafetyCategory.EMERGENCY_PREPAREDNESS]: <AlertTriangle className="h-5 w-5" />,
  ...
  const getCategoryColor = (category: SafetyCategory) => {
    const colors = {
      [SafetyCategory.CRIME_PREVENTION]: 'bg-red-50 border-red-200 text-red-700',
      [SafetyCategory.HOME_SECURITY]: 'bg-blue-50 border-blue-200 text-blue-700',
      [SafetyCategory.PERSONAL_SAFETY]: 'bg-green-50 border-green-200 text-green-700',
      [SafetyCategory.CYBER_SECURITY]: 'bg-purple-50 border-purple-200 text-purple-700',
      [SafetyCategory.TRAFFIC_SAFETY]: 'bg-orange-50 border-orange-200 text-orange-700',
      [SafetyCategory.EMERGENCY_PREPAREDNESS]: 'bg-yellow-50 border-yellow-200 text-yellow-700',
      [SafetyCategory.COMMUNITY_PROGRAMS]: 'bg-indigo-50 border-indigo-200 text-indigo-700',
      [SafetyCategory.AWARENESS_CAMPAIGNS]: 'bg-pink-50 border-pink-200 text-pink-700'
    };
  ...
  const getEmergencyTypeIcon = (type: EmergencyType) => {
    const icons = {
      ...
      [EmergencyType.DISASTER_RESPONSE]: <AlertTriangle className="h-6 w-6 text-orange-600" />,
      [EmergencyType.POISON_CONTROL]: <AlertTriangle className="h-6 w-6 text-purple-600" />,
      [EmergencyType.MENTAL_HEALTH]: <Heart className="h-6 w-6 text-green-600" />,
      [EmergencyType.DOMESTIC_VIOLENCE]: <Shield className="h-6 w-6 text-purple-600" />
    };
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-8">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-gray-600 mt-2">Public safety resources, tips, and emergency contacts</p>
            </div>
          </div>
        </div>
        ...
        <Tabs defaultValue="safety-tips" className="w-full">
          <TabsList className="grid w-full grid-cols-2">
            <TabsTrigger value="safety-tips">Safety Tips & Resources</TabsTrigger>
          ...
          <TabsContent value="safety-tips" className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {filteredSafetyInfo.map((info) => (
                <Card key={info.id} className={`hover:shadow-lg transition-shadow ${getCategoryColor(info.category)} border-l-4`}>
                  <CardContent className="p-6">
                    <div className="flex items-start justify-between mb-3">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        >
                          <Bookmark 
                            className={`h-4 w-4 ${
                              bookmarkedItems.includes(info.id) ? 'fill-current text-yellow-500' : 'text-gray-400'
                            }`} 
                          />
                        </Button>
                      </div>
                    ...
                    <p className="text-gray-600 text-sm mb-4 line-clamp-3">{info.content}</p>
                      ...
                      {info.resources.length > 0 && (
                        <div>
                          ...
                          <div className="flex flex-wrap gap-1">
                            {info.resources.slice(0, 2).map((resource, index) => (
                      ...
                      <Dialog>
                        ...
                        <DialogContent className="max-w-4xl max-h-[80vh] overflow-y-auto">
                          <DialogHeader>
                            ...
                            <DialogDescription>
                              {info.category.replace('_', ' ').toUpperCase()} • Updated {info.lastUpdated.toLocaleDateString()}
                            </DialogDescription>
                          </DialogHeader>
                                ...
                                >
                                  <Bookmark 
                                    className={`h-4 w-4 mr-2 ${
                                      bookmarkedItems.includes(info.id) ? 'fill-current text-yellow-500' : ''
                                    }`} 
                                  />
                                  {bookmarkedItems.includes(info.id) ? 'Bookmarked' : 'Bookmark'}
                                </Button>
                              </div>
                            </div>
                          </div>
            ...
            {filteredSafetyInfo.length === 0 && (
              <Card>
                <CardContent className="p-12 text-center">
                  ...
                  <h3 className="text-lg font-semibold text-gray-900 mb-2">No safety information found</h3>
                  <p className="text-gray-600 mb-4">No safety information matches your search criteria.</p>
                  <Button
                    variant="outline"
                    onClick={() => {
                      setSearchTerm('');
          ...
          <TabsContent value="emergency-contacts" className="space-y-6">
            <Alert className="mb-6">
              ...
              <AlertDescription>
                <strong>In a life-threatening emergency, call 911 immediately.</strong> 
                The contacts below are for non-emergency situations and support services.
              </AlertDescription>
            </Alert>
              ...
              {emergencyContacts
                ...
                .map((contact) => (
                  <Card key={contact.id} className={`hover:shadow-lg transition-shadow ${
                    contact.priority === 1 ? 'ring-2 ring-red-500 bg-red-50' : ''
                  }`}>
                    <CardContent className="p-6">
                        ...
                        <div className="flex-1">
                          ...
                          <div className="space-y-2 text-sm">
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              <span className="text-gray-600">{contact.availableHours}</span>
                            </div>
                            {contact.location && (
                              <div className="flex items-center gap-2">
            ...
            <Card className="bg-blue-50 border-blue-200">
              <CardContent className="p-6">
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    <h3 className="font-semibold text-blue-900 mb-2">Additional Support</h3>
                    <p className="text-blue-800 text-sm mb-3">
                      If you need help but don't see an appropriate contact above, call our main police number 
                      at (555) 123-4567. Our operators can direct you to the right department or service.
                    </p>
                    <p className="text-blue-800 text-sm">
                      For TTY/TDD services, dial 711 before any of the numbers above.
                    </p>
                  </div>
                </div>
              </CardContent>
