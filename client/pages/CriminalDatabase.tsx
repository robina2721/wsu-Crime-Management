...
import { CriminalRecord, RiskLevel, Conviction, ArrestRecord, Warrant, WarrantStatus, UserRole, CrimeCategory } from '@shared/types';
...
export default function CriminalDatabase() {
    ...
    const mockCriminals: CriminalRecord[] = [
      {
        ...
        physicalDescription: {
          ...
          eyeColor: 'Brown',
          hairColor: 'Black',
          distinguishingMarks: ['Scar on left forearm', 'Tattoo on right shoulder']
        },
        criminalHistory: {
          convictions: [
            {
      ...
      },
      {
        ...
        physicalDescription: {
          ...
          eyeColor: 'Brown',
          hairColor: 'Black',
          distinguishingMarks: ['Large tattoo across back', 'Gold tooth (front left)']
        },
        criminalHistory: {
          convictions: [
            {
  ...
  const filterCriminals = () => {
    ...
    if (searchTerm) {
      filtered = filtered.filter(criminal => 
        criminal.personalInfo.fullName.toLowerCase().includes(searchTerm.toLowerCase()) ||
        criminal.personalInfo.aliases?.some(alias => 
          alias.toLowerCase().includes(searchTerm.toLowerCase())
        ) ||
        criminal.personalInfo.nationalId?.toLowerCase().includes(searchTerm.toLowerCase())
      );
    }

    if (riskFilter !== 'all') {
      filtered = filtered.filter(criminal => criminal.riskLevel === riskFilter);
    }
  ...
  const getRiskBadgeColor = (risk: RiskLevel) => {
    switch (risk) {
      ...
      case RiskLevel.HIGH: return 'bg-crime-red text-white';
      case RiskLevel.MEDIUM: return 'bg-crime-yellow text-crime-black';
  ...
  const criminalStats = {
    ...
    highRisk: criminals.filter(c => c.riskLevel === RiskLevel.HIGH || c.riskLevel === RiskLevel.CRITICAL).length,
    activeWarrants: criminals.reduce((count, criminal) => {
      return count + criminal.criminalHistory.warrants.filter(w => w.status === WarrantStatus.ACTIVE).length;
    }, 0)
  };
      ...
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <Card className="w-full max-w-md">
          <CardContent className="p-6 text-center">
            ...
            <p className="text-gray-600">You don't have permission to access the criminal database.</p>
          </CardContent>
        </Card>
      </div>
    );
        ...
        <div className="max-w-7xl mx-auto">
          <div className="flex justify-between items-center">
            <div>
              ...
              <p className="text-gray-300">Secure access to criminal records and investigations</p>
            </div>
      ...
      <div className="max-w-7xl mx-auto p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
          <Card>
            <CardContent className="p-6 text-center">
              ...
              <h3 className="text-2xl font-bold text-crime-black">{criminalStats.total}</h3>
          ...
          <Card>
            <CardContent className="p-6 text-center">
              ...
              <h3 className="text-2xl font-bold text-crime-black">{criminalStats.active}</h3>
          ...
          <Card>
            <CardContent className="p-6 text-center">
              ...
              <h3 className="text-2xl font-bold text-crime-black">{criminalStats.highRisk}</h3>
          ...
          <Card>
            <CardContent className="p-6 text-center">
              ...
              <h3 className="text-2xl font-bold text-crime-black">{criminalStats.activeWarrants}</h3>
        ...
        <Card className="mb-8">
          <CardHeader>
            ...
            <CardDescription>Search by name, alias, or national ID</CardDescription>
          </CardHeader>
          <CardContent>
          ...
          <CardContent>
            <div className="space-y-4">
              {filteredCriminals.map((criminal) => (
                <div key={criminal.id} className="border rounded-lg p-6 hover:shadow-md transition-shadow">
                    ...
                    <div className="flex gap-4 flex-1">
                      ...
                      <div className="flex-1">
                        <div className="flex items-center gap-3 mb-2">
                          ...
                          <Badge className={getRiskBadgeColor(criminal.riskLevel)}>
                            {criminal.riskLevel.toUpperCase()} RISK
                          </Badge>
                          <Badge className={criminal.isActive ? 'bg-red-100 text-red-800' : 'bg-gray-100 text-gray-800'}>
                            {criminal.isActive ? 'ACTIVE' : 'INACTIVE'}
                          </Badge>
                          {criminal.criminalHistory.warrants.some(w => w.status === WarrantStatus.ACTIVE) && (
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 text-sm text-gray-600 mb-3">
                          <div>
                            ...
                            <div>{criminal.personalInfo.dateOfBirth ? calculateAge(criminal.personalInfo.dateOfBirth) : 'Unknown'}</div>
                          </div>
                          <div>
                            <span className="font-medium">National ID:</span>
                            <div>{criminal.personalInfo.nationalId || 'Unknown'}</div>
                          </div>
                          <div>
                            <span className="font-medium">Convictions:</span>
                            <div>{criminal.criminalHistory.convictions.length}</div>
                          </div>
                          <div>
                            <span className="font-medium">Last Updated:</span>
                            <div>{new Date(criminal.lastUpdated).toLocaleDateString()}</div>
                          </div>
                        </div>

                        {criminal.personalInfo.aliases && criminal.personalInfo.aliases.length > 0 && (
                          <div className="mb-3">
                            <span className="text-sm font-medium text-gray-700">Known Aliases: </span>
                            {criminal.personalInfo.aliases.map((alias, index) => (
            ...
            {selectedCriminal && (
              <Tabs defaultValue="personal" className="space-y-4">
                    ...
                    <Card>
                      ...
                      <CardContent className="space-y-3">
                        ...
                        <div>
                          ...
                          <div>{selectedCriminal.personalInfo.dateOfBirth?.toLocaleDateString() || 'Unknown'}</div>
                        </div>
                        <div>
                          <span className="font-medium">National ID:</span>
                          <div>{selectedCriminal.personalInfo.nationalId || 'Unknown'}</div>
                        </div>
                        <div>
                          <span className="font-medium">Address:</span>
                          <div>{selectedCriminal.personalInfo.address || 'Unknown'}</div>
                        </div>
                        <div>
                          <span className="font-medium">Phone:</span>
                          <div>{selectedCriminal.personalInfo.phone || 'Unknown'}</div>
                        </div>
                        <div>
                          <span className="font-medium">Risk Level:</span>
                          <div>
                            <Badge className={getRiskBadgeColor(selectedCriminal.riskLevel)}>
                    ...
                    <Card>
                      ...
                      <CardContent className="space-y-3">
                        <div>
                          ...
                          <div>{selectedCriminal.physicalDescription.height ? `${selectedCriminal.physicalDescription.height} cm` : 'Unknown'}</div>
                        </div>
                        <div>
                          <span className="font-medium">Weight:</span>
                          <div>{selectedCriminal.physicalDescription.weight ? `${selectedCriminal.physicalDescription.weight} kg` : 'Unknown'}</div>
                        </div>
                        <div>
                          <span className="font-medium">Eye Color:</span>
                          <div>{selectedCriminal.physicalDescription.eyeColor || 'Unknown'}</div>
                        </div>
                        <div>
                          <span className="font-medium">Hair Color:</span>
                          <div>{selectedCriminal.physicalDescription.hairColor || 'Unknown'}</div>
                        </div>
                        <div>
                          <span className="font-medium">Distinguishing Marks:</span>
                          <div>
                            {selectedCriminal.physicalDescription.distinguishingMarks?.map((mark, index) => (
                ...
                <TabsContent value="convictions" className="space-y-4">
                  <div className="space-y-4">
                    {selectedCriminal.criminalHistory.convictions.map((conviction) => (
                      <Card key={conviction.id}>
                        <CardContent className="p-4">
                          <div className="flex justify-between items-start mb-3">
                            <h4 className="font-semibold text-crime-black">{conviction.description}</h4>
                            <Badge className="bg-crime-red text-white">
                              {conviction.crimeType.replace('_', ' ').toUpperCase()}
                            </Badge>
                          </div>
                          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                            <div>
                              <span className="font-medium">Date:</span>
                              <div>{conviction.date.toLocaleDateString()}</div>
                            </div>
                            <div>
                          ...
                          <div className="mt-2 text-sm">
                            <span className="font-medium">Case Number:</span> {conviction.caseNumber}
                          </div>
                        </CardContent>
                      </Card>
                    ))}
                    {selectedCriminal.criminalHistory.convictions.length === 0 && (
                      ...
                      <Card key={arrest.id}>
                        <CardContent className="p-4">
                          <div className="flex justify-between items-start mb-3">
                            ...
                            <Badge variant="outline">{arrest.disposition}</Badge>
                          </div>
                ...
                <TabsContent value="warrants" className="space-y-4">
                  <div className="space-y-4">
                    {selectedCriminal.criminalHistory.warrants.map((warrant) => (
                      <Card key={warrant.id}>
                        <CardContent className="p-4">
                          <div className="flex justify-between items-start mb-3">
                            <h4 className="font-semibold text-crime-black">
                              {warrant.type.replace('_', ' ').toUpperCase()} Warrant
                            </h4>
                            <Badge className={
                              warrant.status === WarrantStatus.ACTIVE ? 'bg-red-100 text-red-800' :
                              warrant.status === WarrantStatus.EXECUTED ? 'bg-green-100 text-green-800' :
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                            <div>
                              ...
                              <div>{warrant.issueDate.toLocaleDateString()}</div>
                            </div>
                            <div>
                              <span className="font-medium">Issuing Court:</span>
                              <div>{warrant.issuingCourt}</div>
                            </div>
                          </div>
                          {warrant.expiryDate && (
                            <div className="mt-2 text-sm">
                              <span className="font-medium">Expiry Date:</span> {warrant.expiryDate.toLocaleDateString()}
                            </div>
                          )}
                          <div className="mt-3">
                            <span className="font-medium">Charges:</span>
