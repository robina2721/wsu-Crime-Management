...
export async function listUsers(limit = 100, offset = 0, filters = {}) {
  ...
  const params = [];
  if (filters.role) {
    where.push(`role = @p${params.length + 1}`);
    params.push(filters.role);
  }
  if (typeof filters.isActive === "boolean") {
    where.push(`is_active = @p${params.length + 1}`);
...
export async function listActiveOfficersWithCaseCounts(
  limit = 100,
  offset = 0,
) {
  const activeStatuses = ["reported", "assigned", "under_investigation"];
...
export async function updateUser(id, updates) {
  ...
  params.push(id);
  await queryRows(
    `UPDATE users SET ${set.join(", ")} WHERE id = @p${params.length}`,
    params,
  );
  return await findUserById(id);
}
