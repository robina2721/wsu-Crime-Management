...
export async function listCrimes(filters, limit, offset) {
  ...
  if (filters.assignedTo) { where.push(`assigned_to = @p${params.length + 1}`); params.push(filters.assignedTo); }
...
export async function createCrime(data) {
  ...
  await queryRows(
    ...
    [id, data.title, data.description, data.category, data.priority ?? "medium", data.location, now, new Date(data.dateIncident), data.reportedBy, now, now]
  );
...
export async function updateCrime(id, updates) {
  const now = new Date();
  const mapping = { dateIncident: "date_incident", reportedBy: "reported_by", assignedTo: "assigned_to" };
  const allowed = new Set(["title","description","category","status","priority","location","date_incident","reported_by","assigned_to"]);
  ...
  params.push(now); set.push(`updated_at = @p${params.length}`); params.push(id);
