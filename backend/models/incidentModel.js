...
export async function listIncidents(limit = 100, offset = 0, filters = {}) {
  ...
  if (filters.incidentType) { where.push(`incident_type = @p${params.length + 1}`); params.push(filters.incidentType); }
  if (filters.severity) { where.push(`severity = @p${params.length + 1}`); params.push(filters.severity); }
...
export async function createIncident(data) {
  ...
  await queryRows(
    ...
    [id, data.title, data.description, data.incidentType, data.severity || 'low', data.location, new Date(data.dateOccurred), data.reportedBy, data.reporterName, data.status || 'reported', data.followUpRequired ? 1 : 0, data.relatedCaseId || null, now, now]
  );
  return await getIncident(id);
}

export async function updateIncident(id, updates) {
  const mapping = { incidentType: 'incident_type', dateOccurred: 'date_occurred', reportedBy: 'reported_by', reporterName: 'reporter_name', followUpRequired: 'follow_up_required', relatedCaseId: 'related_case_id' };
  const allowed = new Set(["title","description","incident_type","severity","location","date_occurred","reported_by","reporter_name","status","follow_up_required","related_case_id"]);
  ...
  params.push(now);
  params.push(id);
  await queryRows(`UPDATE incidents SET ${set.join(', ')} WHERE id = @p${params.length}`, params);
  return await getIncident(id);
}
