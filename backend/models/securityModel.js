...
export async function clearFailedAttemptsForUser(userId) {
  await ensureFailedLoginTable();
  await queryRows(`DELETE FROM failed_login_attempts WHERE user_id = @p1`, [
    userId,
  ]);
  return true;
}
...
export async function listAudit(limit = 100, offset = 0, filters = {}) {
  ...
  const params = [];
  if (filters.username) {
    where.push(`username = @p${params.length + 1}`);
    params.push(filters.username);
  }
  if (filters.userId) {
    where.push(`user_id = @p${params.length + 1}`);
    params.push(filters.userId);
  }
  if (typeof filters.success === "boolean") {
    where.push(`success = @p${params.length + 1}`);
