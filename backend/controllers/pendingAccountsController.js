...
import { listPendingAccounts, createPendingAccount, approvePendingAccount, rejectPendingAccount, findPendingById } from "../../backend/models/pendingAccountModel.js";
...
export async function listPendingHandler(req) {
  try {
    ...
    const rows = await listPendingAccounts(limit, offset, { status, requestedRole });
    return NextResponse.json({ success: true, data: { pending: rows, total: rows.length, limit, offset } });
  } catch (err) {
...
export async function requestAccountHandler(req) {
  try {
    ...
    return NextResponse.json({ success: true, data: created, message: "Account request submitted" }, { status: 201 });
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    const status = msg.includes("SQL Server not configured") ? 503 : 500;
    return NextResponse.json({ success: false, error: msg }, { status });
...
export async function approvePendingHandler(req, params) {
  try {
    ...
    if (!pending) return NextResponse.json({ success: false, error: "Pending account not found" }, { status: 404 });
    const body = await req.json().catch(() => ({}));
    const res = await approvePendingAccount(params.id, body || {});
    return NextResponse.json({ success: true, data: res, message: "Pending account approved" });
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    const status = msg.includes("SQL Server not configured") ? 503 : 500;
    return NextResponse.json({ success: false, error: msg }, { status });
...
export async function rejectPendingHandler(req, params) {
  try {
    ...
    return NextResponse.json({ success: true, data: updated, message: "Pending account rejected" });
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    const status = msg.includes("SQL Server not configured") ? 503 : 500;
    return NextResponse.json({ success: false, error: msg }, { status });
