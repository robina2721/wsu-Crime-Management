...
import { listIncidents, getIncident, createIncident, updateIncident, deleteIncident } from "../../backend/models/incidentModel.js";
...
export async function listIncidentsHandler(req) {
  try {
    ...
    const rows = await listIncidents(limit, offset, { status, incidentType, severity });
    return NextResponse.json({ success: true, data: { incidents: rows, total: rows.length, limit, offset } });
  } catch (err) {
...
export async function getIncidentHandler(_req, params) {
  try {
    ...
    if (!it) return NextResponse.json({ success: false, error: "Incident not found" }, { status: 404 });
...
export async function createIncidentHandler(req) {
  try {
    ...
    if (!allow(["citizen","preventive_officer","detective_officer","police_head","super_admin"], user)) {
...
export async function updateIncidentHandler(req, params) {
  try {
    ...
    if (!updated) return NextResponse.json({ success: false, error: "Incident not found" }, { status: 404 });
    return NextResponse.json({ success: true, data: updated, message: "Incident updated" });
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    const status = msg.includes("SQL Server not configured") ? 503 : 500;
    return NextResponse.json({ success: false, error: msg }, { status });
